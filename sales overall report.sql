...
LEFT JOIN (
    ...
    FROM
        `tabPurchase Order Item` latest_po
    JOIN
        `tabPurchase Order` po ON latest_po.parent = po.name
    WHERE
        ...
        AND po.creation = (
            SELECT
                MAX(po2.creation)
            FROM
                `tabPurchase Order Item` poi2
            JOIN
                `tabPurchase Order` po2 ON poi2.parent = po2.name
            WHERE
                poi2.item_code = latest_po.item_code
