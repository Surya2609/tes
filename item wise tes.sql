SELECT
    ...
    IFNULL(item.custom_rol_qty, 0) AS `Reorder Level Qty`,
    IFNULL(item.custom_rol_qty, 0) + so_summary.so_remaining_qty AS `Total Qty`,
    ...
    CASE
        WHEN 
            (IFNULL(item.custom_rol_qty, 0) + so_summary.so_remaining_qty)
