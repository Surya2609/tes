frappe.ui.form.on("Service DC IN", {
  ...
  validate: function (frm) {
      console.log("invokedddddddddddddd");
    let allValid = true;
    frm.doc.items.forEach((row, index) => {

        //////////////////////////////////////////////////////////////// changed updated

      let rej_qty = parseFloat(row.rejected_qty) || 0;
      let accepeted_qty = parseFloat(row.qty) || 0;
      ...
      if (givenQty > 0) {
        if (
          ...
          frappe.msgprint(
            __(
              "Row {0}: Please fill all mandatory fields (Qty, Server For, Source Warehouse, Target Warehouse, Item).",
              [index + 1]
            )
          );
          allValid = false;
             ...
             if (rej_qty > 0 && !row.rejected_warehouse) {
                frappe.msgprint(
                  __("Row {0}: Please specify Rejected Warehouse when Rejected Qty is greater than 0.", [index + 1])
                );
                allValid = false;
              }
  ...
  before_submit: function (frm) {
    ...
    if (frm.doc.dc_status !== "Completed") {
      frappe.msgprint(
        __('Cannot submit because the status is not "Completed".')
      );
      ...
      if (rej_qty > 0) {
        if (item.qty > 0) {
          ...
          items.push({
            ...
            s_warehouse: item.source_warehouse, // Source warehouse
            t_warehouse:
              item.rejected_warehouse !== null &&
              item.rejected_warehouse !== undefined
                ? item.rejected_warehouse
                : frm.doc.rejected_warehouse !== null &&
                    ...
                    frappe.msgprint(
                      __(
                        "Stock Entry submitted successfully: " +
                          submit_response.message.name
                      )
                    );

                    try {
                      await get_submited_st_entry(
                        frm,
                        submit_response.message.name
                      );
    ...
    frappe.call({
      ...
      callback: function (get_response) {
        if (get_response.message && Array.isArray(get_response.message)) {
          ...
          frm.doc.items.forEach(function (row) {
            ...
            get_response.message.forEach(function (st_entry_item) {
              if (
                ...
                calculation_part(
                  row,
                  st_entry_item.transfer_qty,
                  name,
                  st_entry_item.t_warehouse
                );
              }
            });
          });
          ...
          frappe.msgprint(
            __(
              "Failed to fetch Stock Entry details: " +
                JSON.stringify(get_response.message)
            )
          );
...
function calculation_part(
  row_item,
  transfer_qty,
  new_stock_tr_id,
  stock_entred_warehouse
) {
  let totalQty = parseFloat(row_item.total_qty) || 0;
  let balanceQty = parseFloat(row_item.balance_qty) || 0;
  ...
  if (stock_entred_warehouse == "Rejected Items - MVDF") {
    ...
    let accepeted_qty =
      givenQty + (parseFloat(row_item.total_accepted_qty) || 0);
    frappe.model.set_value(
      row_item.doctype,
      row_item.name,
      "total_accepted_qty",
