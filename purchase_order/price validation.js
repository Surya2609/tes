...
function withOutSuppPrice(item) {
    frappe.call({
        method: 'get_po_rate_with_out_supplier', // Replace with the correct path to your method
        args: {
            item_code: item.item_code,
            uom: item.uom
        },
        callback: function (r) {
            ...
            if (r.message && r.message.length > 0) {
                let data = r.message[0];
                console.log("Fetched without supplier :", data);
