SELECT
    ...
    CASE
        WHEN 
            ...
            - (SUM(IFNULL(bin.actual_qty, 0)) + IFNULL(transit_summary.transit_qty, 0) + (IFNULL(po_summary.po_remaining_qty_in_base_uom, 0) - IFNULL(transit_summary.transit_qty, 0))) > 0
        THEN 
            (IFNULL(reorder_summary.total_reorder_level, 0) + so_summary.so_remaining_qty)
            - (SUM(IFNULL(bin.actual_qty, 0)) + IFNULL(transit_summary.transit_qty, 0) + (IFNULL(po_summary.po_remaining_qty_in_base_uom, 0) - IFNULL(transit_summary.transit_qty, 0)))
        ELSE 0
...
FROM (
    ...
    WHERE
        (%(company)s IS NULL OR so.company = %(company)s)
        AND (
            so.custom_order_status != 'Fore Cast' AND so.custom_order_status != 'NPD'
        )
    GROUP BY
        so.company, soi.item_code
) AS so_summary
