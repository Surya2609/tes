...
async def store_employees(data: EmployeeList):
    ...
    for employee in data.employees:
        if employee.image:
            ...
            if encodings:
                stored_employees.append({"name": employee.name, "employee_name": employee.employee_name, "company" : employee.company, "encoding": encodings[0].tolist()})
            else:
                stored_employees.append({"name": employee.name,  "employee_name": employee.employee_name, "company" : employee.company, "encoding": None})
...
async def recognize_employee(data: ImageData):
    ...
    for employee in employee_list:
        if employee["encoding"] is not None:
            ...
            employee_companies.append(employee["company"])
    ...
    if best_distance < THRESHOLD:
        return {
            ...
            "company": employee_companies[best_match_index],
