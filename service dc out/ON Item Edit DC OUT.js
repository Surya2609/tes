frappe.ui.form.on('Service DC OUT Items', {
    item: function (frm, cdt, cdn) {
        ...
        if (!frm.doc.company) {
            frappe.msgprint(__('Selected Company First!'));
        }
                ...
                callback: function (r) {
                    if (r.message && r.message.length > 0) {
                        ...
                        if (selectedWarehouse) {
                            ...
                            frm.refresh_field('items');
                            // frappe.show_alert({
                            //     message: __("Quantity set to {0} for warehouse {1}", [selectedWarehouse.stock_in_warehouse, row.source_warehouse]),
                            //     indicator: "green"
                            // });
                        } else {
