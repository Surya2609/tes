...
def execute(filters=None):
    ...
    columns = [
        ...
        # {"label": "base Pending Qty", "fieldname": "dn_current_pending_qty", "fieldtype": "Float", "width": 100},
    ...
    platting_stock AS (
        ...
        LEFT JOIN (
            SELECT lft, rgt FROM tabWarehouse WHERE name = 'Non Moving Warehouse Quality - MVDF' LIMIT 1
        ) excluded_w ON w.lft >= excluded_w.lft AND w.rgt <= excluded_w.rgt
        WHERE excluded_w.lft IS NULL
        GROUP BY b.item_code
    ),
    ...
    for row in raw_data:
        ...
        # ✅ Fix: update the row so UI reflects the remaining platting
        row["platting_stock_qty"] = platting_tracker[item]
        ...
        <div style='
            ...
            gap: 16px;
        '>
            <button onclick="alert('Available Stock button is pressed')" style='all:unset; cursor:pointer; flex:1; font-size:20px; font-weight:bold; color:#155724; background-color:#d4edda; padding:10px; border-radius:8px; min-width:200px;'>
            ...
            <div style='flex:1; font-size:20px; font-weight:bold; color:#004085; background-color:#cce5ff; padding:10px; border-radius:8px; min-width:200px;'>
                🔵 Platting Stock: <span style='color:#c82333;'>{blue_count}</span>
            </div>
        </div>
        """]
