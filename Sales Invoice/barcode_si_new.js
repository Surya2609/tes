frappe.ui.form.on('Sales Invoice', {
    refresh: function (frm) {
        frm.add_custom_button('Generate Label Test', async function () {

            let all_qty_by_code = {};
            let batch_wise_items = {};

            // First loop: calculate total qty for each item_code
            frm.doc.items.forEach(item => {
                let code = item.item_code;
                let qty = parseFloat(item.qty) || 0;
                all_qty_by_code[code] = (all_qty_by_code[code] || 0) + qty;
            });

            // Second loop: group by item_code + batch_no + warehouse
            frm.doc.items.forEach((item, index) => {
                let code = item.item_code;
                let batch = item.batch_no || 'N/A';
                let warehouse = item.warehouse || 'N/A';
                let key = `${code}_${batch}_${warehouse}`;

                if (!batch_wise_items[key]) {
                    batch_wise_items[key] = {
                        item_code: code,
                        custom_customer_part_code: item.custom_customer_part_code,
                        batch_no: batch,
                        warehouse: warehouse,
                        uom: item.uom || 'N/A',
                        qty: all_qty_by_code[code],
                        index: index
                    };
                }
            });

            let items = Object.values(batch_wise_items);
